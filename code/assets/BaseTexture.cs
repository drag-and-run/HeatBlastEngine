            ...
            switch (_type)
            {
                case TextureType.Cubemap:
                    for (int i = 0; i < 6; i++)
                    {
                        ...
                        ImageResult img = ImageResult.FromMemory(System.IO.File.ReadAllBytes(path),
                            ColorComponents.RedGreenBlue);

                        fixed (byte* ptr = img.Data)
                        {
                            _gl.TexImage2D(TextureTarget.TextureCubeMapPositiveX + i, 0, InternalFormat.Rgb,
                ...
                case TextureType.Color:
                    Path = filepath;
                    ImageResult result = ImageResult.FromMemory(System.IO.File.ReadAllBytes(filepath[0]),
